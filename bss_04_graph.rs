...
fn _assert_serde() {
    fn _check<T: serde::Serialize + for<'de> serde::Deserialize<'de>>() {}
}
...
pub struct GraphPersistence {
    pub adjacency_list: Arc<DashMap<String, Vec<Neighbor>>>,
}
...
impl GraphPersistence {
    ...
    pub fn validate_global_invariants(&self) -> Option<String> {
        for entry in self.adjacency_list.iter() {
            ...
            for neighbor in entry.value().iter() {
                ...
                if neighbor.pool.reserve_0 < MIN_RESERVE_THRESHOLD
                    || neighbor.pool.reserve_1 < MIN_RESERVE_THRESHOLD
                {
                    return Some(format!(
                        "Dust liquidity detected in pool {}",
                        neighbor.pool.pool_address
                    ));
                }
            }
        }
    ...
    fn add_neighbor(&self, from: String, to: String, state: PoolState) {
        ...
        if state.reserve_0 < MIN_RESERVE_THRESHOLD || state.reserve_1 < MIN_RESERVE_THRESHOLD {
            return;
        }
        if state.fee_bps > MAX_FEE_BPS {
            return;
        }
        ...
        if let Some(neighbor) = entry
            .iter_mut()
            .find(|n| n.pool.pool_address == state.pool_address)
        {
            neighbor.pool = state;
        } else {
            entry.push(Neighbor {
...
impl crate::SubsystemSpecialist for GraphPersistence {
    ...
    fn upgrade_strategy(&self) -> &'static str {
        "Immutable: Requires process restart to clear memory."
    }

    fn testing_strategy(&self) -> &'static str {
        "Invariant: Verify no negative cycles exist after update."
    }
    ...
    fn check_health(&self) -> crate::HealthStatus {
        ...
        if self.adjacency_list.is_empty() {
            return crate::HealthStatus::Degraded(
                "Graph is empty; check BSS-05 Sync Layer.".into(),
            );
        }
        ...
        for entry in self.adjacency_list.iter() {
            if entry
                .value()
                .iter()
                .any(|n| n.token_address == *entry.key())
            {
                return crate::HealthStatus::Degraded(
                    "Self-referencing loop detected in graph state.".into(),
                );
            }
        }
